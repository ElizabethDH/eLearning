...
elseif ($_SESSION['leveluser']=='dosen'){
    $main = mysqli_query($konek,"SELECT * FROM mainmenu WHERE aktif = 'Y' AND levelmenu= 'D' order by id_main");    
    while($r = mysqli_fetch_array($main)) {
		if(empty($r[link])){
...
elseif ($_SESSION['leveluser']=='mahasiswa'){
    $main = mysqli_query($konek,"SELECT * FROM mainmenu WHERE aktif = 'Y' AND levelmenu= 'M' order by id_main");    
    while($r = mysqli_fetch_array($main)) {
		if(empty($r[link])){
        echo '<li><a class="dropdown"href="#">'.$r[nama_menu].'</a>';
		$sub = mysqli_query($konek, "SELECT * FROM submenu, mainmenu WHERE submenu.id_main = mainmenu.id_main AND submenu.id_main = $r[id_main] 
